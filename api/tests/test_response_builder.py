...
class TestAnswerTextCleanup:
    def test_a_collapsed_table_is_put_back_onto_its_rows(self):
        """Seen in production: the model wrote the whole table on one line, so
        the reader got a paragraph of pipes. The browser needs real rows before
        GFM can draw a table."""
        raw = {
            "query_type": "data",
            "answer": ("The top items account for £12,795,000.00.\n"
                       "| Rank | Ref | Saving | "
                       "| :---: | :---: | ---: | "
                       "| 1 | 42 | £4,700,000.00 | "
                       "| 2 | 145 | £2,000,000.00 |"),
        ...
        lines = build_chat_response(raw).assistant_text.split("\n")
        assert lines[2] == "| Rank | Ref | Saving |"
        assert lines[-1] == "| 2 | 145 | £2,000,000.00 |"

    def test_the_double_negative_cleanup_still_applies(self):
        raw = {"query_type": "document",
               "answer": "The report does not not mention the delay in detail.",
               "sources": [{"file_name": "d.pdf"}]}
        assert "does not mention" in build_chat_response(raw).assistant_text
...
class TestSQLArtifact:
    ...
    def test_columns_keep_the_query_order(self):
        """Object.keys(rows[0]) on the client loses the SELECT order, and says
        nothing at all when the result is empty."""
        raw = {
            "query_type": "data",
            "answer": "result",
            "sources": [{"type": "structured_data", "table_name": "t"}],
            "sql": "SELECT ref, saving FROM t",
            "result_data": [{"saving": 100, "ref": "42"}],
            "result_columns": ["ref", "saving"],
        }
        resp = build_chat_response(raw)
        assert resp.sql_artifact.columns == ["ref", "saving"]

    def test_columns_fall_back_to_the_first_row(self):
        raw = {
            "query_type": "data",
            "answer": "result",
            "sources": [{"type": "structured_data", "table_name": "t"}],
        ...
        resp = build_chat_response(raw)
        assert resp.sql_artifact.columns == ["a", "b"]
