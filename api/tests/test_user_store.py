...
def test_find_user_maps_synthetic_auth_email(store: UserStore, monkeypatch):
    monkeypatch.setenv("SUPABASE_AUTH_EMAIL_DOMAIN", "users.coair.local")
    store.create_user("ops", "pw")
    found = store.find_user("ops@users.coair.local")
    assert found is not None
    assert found["username"] == "ops"
    assert store.verify_password("ops@users.coair.local", "pw") is not None
...
def test_list_billing_summaries_matches_per_user_lookup(store: UserStore):
    store.create_user("a", "pw", token_limit=1000)
    store.create_user("b", "pw", token_limit=500)
    store.increment_usage("a", prompt_tokens=10, completion_tokens=5)
    batched = store.list_billing_summaries()
    assert set(batched) == {"a", "b"}
    assert batched["a"] == store.get_billing_summary("a")
    assert batched["b"] == store.get_billing_summary("b")


def test_bump_token_epoch(store: UserStore):
    store.create_user("ivy", "pw")
    assert store.get_user("ivy")["token_epoch"] == 0
    assert store.bump_token_epoch("ivy") == 1
    assert store.get_user("ivy")["token_epoch"] == 1
    with pytest.raises(ValueError):
        store.bump_token_epoch("missing")
