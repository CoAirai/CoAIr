...
def test_v2_dates_require_verbatim_cited_content():
    from src.chronology_v2 import _prune_source_invalid_events
    evidence = [EvidenceItem(
        "s1", "d1", "notice.pdf", page=1,
        excerpt="Date: 14 March 2025. Period ending March 2025. Engineer issued notice.",
    )]
    claim = {"text": "Engineer issued notice", "source_ids": ["s1"]}
    valid = [
        {"event_date": "2025-03-14", "date_precision": "exact",
         "date_source": "content_header", "date_evidence": "14 March 2025"},
        {"event_date": "2025-03", "date_precision": "month",
         "date_source": "table_period", "date_evidence": "March 2025"},
    ]
    invalid = [
        {"event_date": "2025-03-14", "date_precision": "exact",
         "date_source": "metadata", "date_evidence": "14 March 2025"},
        {"event_date": "14 March 2025", "date_precision": "exact",
         "date_source": "content_header", "date_evidence": "14 March 2025"},
        {"event_date": "2025-03-14", "date_precision": "exact",
         "date_source": "content_header", "date_evidence": ""},
        {"event_date": "2025-03-14", "date_precision": "exact",
         "date_source": "content_header", "date_evidence": "15 March 2025"},
    ]
    result = _prune_source_invalid_events({"entries": [
        {**date, "claims": [claim]} for date in [*valid, *invalid]
    ]}, evidence)

    assert [event["event_date"] for event in result[:2]] == ["2025-03-14", "2025-03"]
    assert all(event["event_date"] == "" for event in result[2:])
    assert all(event["date_source"] == "unresolved" for event in result[2:])


def test_v2_exact_duplicates_preserve_observations_without_reassigning_sources(monkeypatch):
    from copy import deepcopy
    from src.chronology_v2 import synthesize

    evidence = [
        EvidenceItem("s1", "d1", "letter.pdf", excerpt="Engineer issued notice"),
        EvidenceItem("s2", "d2", "minutes.pdf", excerpt="Engineer issued notice"),
    ]
    candidates = [
        {"event_date": "", "claims": [{"text": "Engineer issued notice", "source_ids": [sid]}]}
        for sid in ("s1", "s2")
    ]
    ...
    def fake(prompt, **kwargs):
        ...
        if kwargs["schema_name"] == "chronology_v2_overview":
            return SimpleNamespace(raw={
                "overview_claims": [{"text": "Engineer issued notice", "source_ids": ["s1"]}],
            })
        cluster = json.loads(
            prompt.split("EVENT CLUSTERS:\n", 1)[1].split("\n\nEVIDENCE BEGIN", 1)[0]
        )[0]
        return SimpleNamespace(raw={"entries": [{
            "cluster_id": cluster["cluster_id"], "event_date": "",
            "date_precision": "unknown", "date_source": "unresolved", "date_evidence": "",
            "claims": [{"text": "Engineer issued notice", "source_ids": ["s1", "s2"]}],
        }]})

    monkeypatch.setattr("src.llm_client.generate_response_json", fake)
    prepared = PreparedChronologyQuery("q", "q", (), (), (), (), (), ("q",))
    result = synthesize(
        prepared=prepared, candidates=candidates, evidence=evidence, cache_context="c",
    )

    assert candidates == original
    assert '"observations"' in captured["prompt"]
    assert "s1" in captured["prompt"] and "s2" in captured["prompt"]
    assert result["entries"][0]["claims"][0]["source_ids"] == ["s1", "s2"]


def test_v2_extraction_checkpoint_hash_includes_prompt_contract(monkeypatch):
    evidence = [EvidenceItem("s1", "d1", "a.pdf", excerpt="event")]
    prepared = PreparedChronologyQuery("q", "q", (), (), (), (), (), ("q",))
    hashes = []
    monkeypatch.setattr("src.chronology_v2.extract_batch", lambda **_kwargs: [])

    for prompt_hash in ("prompt-a", "prompt-b"):
        monkeypatch.setattr("src.chronology_v2.chronology_prompt_hash", lambda: prompt_hash)
        extract_batches(
            evidence, prepared,
            load_step=lambda _key, input_hash: hashes.append(input_hash),
        )
...
def test_failed_batches_record_a_stable_error_code_without_provider_text(monkeypatch):
    ...
    assert error == "model_output_invalid"
    assert stats["batch_errors"][0]["error_code"] == "model_output_invalid"
    assert "too many items" in stats["batch_errors"][0]["detail"]
...
def test_v2_synthesis_keeps_every_cluster_around_the_old_limit(monkeypatch, event_count):
    from src.chronology_v2 import SYNTHESIS_BATCH_EVENTS, synthesize

    def label(index: int) -> str:
        return chr(97 + index // 26) + chr(97 + index % 26)

    evidence = [
        EvidenceItem(f"s-{label(i)}", f"d-{label(i)}", f"{label(i)}.pdf",
                     excerpt=f"event {label(i)}")
        for i in range(event_count)
    ]
    candidates = [
        {"event_date": "", "date_precision": "unknown", "date_source": "unresolved",
         "date_evidence": "", "claims": [{
             "text": f"event {label(i)}", "source_ids": [f"s-{label(i)}"],
         }]}
        for i in range(event_count)
    ]
    calls = []
    ...
    def fake(prompt, **kwargs):
        ...
        if kwargs["schema_name"] == "chronology_v2_overview":
            return SimpleNamespace(raw={
                "overview_claims": [{"text": "event aa", "source_ids": ["s-aa"]}],
            })
        raw = prompt.split("EVENT CLUSTERS:\n", 1)[1].split("\n\nEVIDENCE BEGIN", 1)[0]
        clusters = json.loads(raw)
        return SimpleNamespace(raw={"entries": [{
            "cluster_id": event["cluster_id"], "event_date": "",
            "date_precision": "unknown", "date_source": "unresolved", "date_evidence": "",
            "claims": event["claims"], "parties": [], "event_type": "event",
    ...
    assert calls.count("chronology_v2_event_batch") == (
        event_count + SYNTHESIS_BATCH_EVENTS - 1
    ) // SYNTHESIS_BATCH_EVENTS
    assert {event["cluster_id"] for event in result["entries"]}
...
def test_a_draft_within_the_ceiling_is_untouched(monkeypatch):
    ...
    def fake(prompt, **kwargs):
        if kwargs["schema_name"] == "chronology_v2_overview":
            return SimpleNamespace(raw={
                "overview_claims": [{"text": "delay 2007", "source_ids": ["s1"]}],
            })
        cluster = json.loads(
            prompt.split("EVENT CLUSTERS:\n", 1)[1].split("\n\nEVIDENCE BEGIN", 1)[0]
        )[0]
        return SimpleNamespace(raw={"entries": [{
            "cluster_id": cluster["cluster_id"], "event_date": "2007-03-01",
            "date_precision": "exact", "date_source": "content_body",
            "date_evidence": "delay 2007",
...
def test_v2_verifier_batches_more_than_thirty_claims_without_omission(monkeypatch):
    from src.chronology_v2 import verify_claims
    evidence = [EvidenceItem("s1", "d1", "a.pdf", excerpt="supported event")]
    chronology = {
        "overview_claims": [{"text": "supported event", "source_ids": ["s1"]}],
        "entries": [{
            "claims": [{"text": "supported event", "source_ids": ["s1"]}],
        } for _ in range(40)],
    }
    batches = []
    ...
    def fake(prompt, **_kwargs):
        ...
        batches.append([claim["claim_ref"] for claim in claims])
        return SimpleNamespace(raw={"decisions": [{
            "claim_ref": claim["claim_ref"], "supported": True, "reason_code": "supported",
        } for claim in claims]})

    monkeypatch.setattr("src.llm_client.generate_response_json", fake)
    prepared = PreparedChronologyQuery("q", "q", (), (), (), (), (), ("q",))
    result = verify_claims(
        prepared=prepared, chronology=chronology, evidence=evidence, cache_context="c",
    )

    assert len(batches) == 2
    assert len(result) == 41 and all(result.values())
