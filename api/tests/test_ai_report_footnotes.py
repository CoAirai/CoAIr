...
def test_counter_evidence_is_a_separate_labelled_chronology_footnote():
    evidence = [
        EvidenceItem("support", "D1", "notice.pdf", page=1),
        EvidenceItem("counter", "D2", "reply.pdf", page=2),
    ]
    blob, audit = build_ai_chronology_docx(
        project_name="Demo", issue_number=1, title="Counter evidence",
        entries=[ChronologyEntry(
            entry_ref="", event_date="2025-01-01", date_precision="exact",
            claims=[VerifiedClaim(
                "The contractor issued notice.", ["support"],
    ...
    doc = Document(io.BytesIO(blob))
    assert any(p.text.startswith("Counter-evidence:") for p in doc.paragraphs)
    with zipfile.ZipFile(io.BytesIO(blob)) as package:
        footnotes = ET.fromstring(package.read("word/footnotes.xml"))
    text = " ".join("".join(node.itertext()) for node in footnotes)
    assert "cited as counter-evidence" in text
    assert audit.footnote_records == 2


def test_word_renderer_keeps_all_forty_verified_events():
    evidence = [EvidenceItem("source", "D1", "record.pdf", page=1)]
    entries = [ChronologyEntry(
        entry_ref=f"ignored-{index}", event_date=f"2025-02-{index + 1:02d}",
        date_precision="exact", claims=[VerifiedClaim("Verified event.", ["source"])],
    ) for index in range(28)] + [ChronologyEntry(
        entry_ref=f"ignored-late-{index}", event_date=f"2025-03-{index + 1:02d}",
        date_precision="exact", claims=[VerifiedClaim("Verified event.", ["source"])],
    ) for index in range(12)]

    blob, audit = build_ai_chronology_docx(
        project_name="Demo", issue_number=4, title="Full record",
        entries=entries, evidence=evidence,
    )

    render = validate_ai_chronology_docx(blob, expected_entries=40)
    assert render["paragraphs"] == 40
    assert audit.footnote_records == 40
