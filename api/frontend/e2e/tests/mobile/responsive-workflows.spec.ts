...
const chronologyJob = {
  ...
  error: null, retryable: false, pipeline_version: 'chronology-v3', coverage_status: 'partial',
  created_at: '2026-08-06T00:00:00Z', sequence_number: 1,
  report_url: '/chronology/reports/job-mobile',
  result: {
    coverage_status: 'partial', partial_reasons: ['uncovered_facets'],
    entries: [{ event_date: '2026-01-10', claims: [{ text: 'The Engineer issued Notice 88.', source_ids: ['SRC-PDF'], counter_source_ids: ['SRC-XLS'] }] }],
    evidence: [
...
async function installMobileApi(page: Page) {
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (path === '/api/event-index/status') return fulfill({ states: { ready: 2, processing: 1 }, observation_count: 42, cluster_count: 31, complete: false, partial_reasons: ['event_index_processing'] });
...
test('Chronology sources open in the shared responsive PDF and spreadsheet viewer', async ({ page }) => {
  ...
  await expect(page.getByText('Partial evidence coverage')).toBeVisible();
...
test('Chronology visibly distinguishes searchable documents from incomplete event memory', async ({ page }) => {
  await page.goto('/chronology');
  await expect(page.getByText('Event memory is still building')).toBeVisible();
  await expect(page.getByText(/project remains searchable/i)).toBeVisible();
  await expect(page.getByText(/event index processing/i)).toBeVisible();
});
