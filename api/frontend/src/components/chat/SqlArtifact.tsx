              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <button
                  onClick={() => {
                    ...
                    const cols = artifact.columns?.length
                      ? artifact.columns
                      : Object.keys(rows[0]);
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-[var(--border)]">
                      {/* The query's column order when the backend sent it;
                          the first row's keys only as a fallback for answers
                          stored before that field existed. */}
                      {(artifact.columns?.length
                        ? artifact.columns
                        : Object.keys(artifact.preview_rows[0])
                      ).map((col) => (
                        <th
                          key={col}
                  ...
                  <tbody>
                    {artifact.preview_rows.map((row, i) => (
                      ...
                      >
                        {/* Indexed by the same column list as the header, so a
                            row whose key order differs cannot shift its cells
