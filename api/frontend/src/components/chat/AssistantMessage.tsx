...
const markdownComponents: Components = {
  ...
  thead: ({ children, ...props }) => (
    // Sticky against the scroll box in MarkdownTable, so the column names stay
    // readable on a long table. Needs an opaque background to cover the rows.
    <thead className="sticky top-0 z-10 bg-[var(--bg-primary)]" {...props}>{children}</thead>
  ),
  th: ({ children, ...props }) => (
    <th className="px-3 py-2 text-left text-[10px] font-semibold text-[var(--text-muted)] uppercase tracking-wider border-b border-[var(--border)] whitespace-nowrap" {...props}>
      {children}
    </th>
  ),
  td: ({ children, ...props }) => (
    // Description columns run long; capping the cell keeps the table readable
    // instead of one line stretching it past any useful scroll distance.
    <td className="px-3 py-2 align-top max-w-[46ch] text-[var(--text-primary)] border-b border-[var(--border)]/50" {...props}>
      {children}
    </td>
  ),
  tr: ({ children, ...props }) => (
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="px-4 py-4 md:px-5 md:py-4 assistant-card text-sm leading-relaxed">
            ...
            <div className="prose prose-invert prose-sm max-w-none text-[var(--text-primary)]">
              {/* GFM is what turns a "| a | b |" answer into a real table —
                  without it react-markdown parses plain CommonMark, which has
                  no tables, and the styles above never fire. */}
