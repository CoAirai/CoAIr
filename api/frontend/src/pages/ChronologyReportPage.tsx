...
export default function ChronologyReportPage() {
  ...
  const coverageStatus = String(job?.result?.coverage_status || job?.coverage_status || '');
  const partialReasons = Array.isArray(job?.result?.partial_reasons) ? job.result.partial_reasons as string[] : [];
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="max-w-5xl mx-auto px-3 py-5 sm:px-4 md:px-8 md:py-8">
        ...
        {job.status === 'ready' && coverageStatus === 'partial' && <section role="status" className="mt-7 border border-[var(--warning)] bg-[var(--wash)] p-4 text-[11px] leading-5 text-[var(--text-secondary)]"><p className="font-semibold text-[var(--text-primary)]">Partial evidence coverage</p><p className="mt-1">This chronology reports only the evidence successfully retrieved and processed.</p>{partialReasons.length > 0 && <ul className="mt-2 list-disc pl-5">{partialReasons.map((reason) => <li key={reason}>{reason.replaceAll('_', ' ')}</li>)}</ul>}</section>}

        {job.status === 'ready' && <div className="mt-8 grid lg:grid-cols-[1fr_280px] gap-6">
          <main className="min-w-0 space-y-4">{entries.map((entry, index) => <article key={index} className="min-w-0 border border-[var(--border)] bg-[var(--bg-primary)] p-4 md:p-5">
            <p className="break-words font-mono text-[10px] text-[var(--accent)]">6.{job.sequence_number}.{index + 1} · {String(entry.event_date || (index === 0 ? 'Overview' : 'Date not established'))}</p>
            {(Array.isArray(entry.claims) ? entry.claims as Array<Record<string, unknown>> : []).map((claim, claimIndex) => <div key={claimIndex}>
              <p className="mt-3 break-words text-[13px] leading-6 text-[var(--text-primary)]">{String(claim.text || '')} <span className="inline-flex flex-wrap gap-1">{(Array.isArray(claim.source_ids) ? claim.source_ids as string[] : []).map((id) => <button type="button" key={id} onClick={() => void showSource(id)} className="inline-flex min-h-11 items-center font-mono text-[9px] text-[var(--accent)] hover:underline md:min-h-0">[{id}]</button>)}</span></p>
              {Array.isArray(claim.counter_source_ids) && claim.counter_source_ids.length > 0 && <p className="mt-2 break-words text-[11px] leading-5 text-[var(--text-secondary)]"><span className="font-semibold">Counter-evidence:</span> <span className="inline-flex flex-wrap gap-1">{(claim.counter_source_ids as string[]).map((id) => <button type="button" key={id} onClick={() => void showSource(id)} className="inline-flex min-h-11 items-center font-mono text-[9px] text-[var(--accent)] hover:underline md:min-h-0">[{id}]</button>)}</span></p>}
            </div>)}
          </article>)}</main>
