                      ...
                      <div key={file.id} className="group flex min-h-14 items-center gap-3 border-b border-[var(--border)] last:border-b-0 bg-[var(--bg-primary)] px-3 py-2 md:px-4 md:py-3">
                        ...
                        <span className="min-w-0 flex-1">
                          ...
                          <span className="mt-1 flex gap-2 font-mono text-[8px] uppercase tracking-[.08em] text-[var(--text-muted)] md:hidden">
                            ...
                            {file.event_index_status && file.event_index_status !== 'not_started' && <><span aria-hidden="true">·</span><span>events {file.event_index_status}{typeof file.event_observation_count === 'number' && file.event_observation_count > 0 ? ` · ${file.event_observation_count}` : ''}</span></>}
                          </span>
                        </span>
                        <span className="hidden md:block font-mono text-[9px] uppercase text-[var(--text-muted)]">{file.event_index_status && file.event_index_status !== 'not_started' ? `events ${file.event_index_status}` : (file.status ?? 'ready')}</span>
                        {canEditFiles && (
