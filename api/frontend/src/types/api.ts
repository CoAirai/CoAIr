...
export interface SQLArtifact {
  ...
  /** The query's own column order. Older stored answers may not carry it. */
  columns?: string[];
...
export interface FileInfo {
  ...
  event_index_status?: 'not_started' | 'queued' | 'processing' | 'ready' | 'partial' | 'failed' | 'stale';
  event_observation_count?: number;
  event_cluster_count?: number;
  event_index_version?: string;
  event_partial_reasons?: string[];
  pages: number | null;
  ocr_pages: number;
  tables: number;
  rows: number;
