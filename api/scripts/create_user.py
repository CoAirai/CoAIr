...
To bootstrap the top-tier account (the only role allowed to manage other
admins), or to promote an existing admin without touching its password:
    python scripts/create_user.py --username owner --password 'owner-pw' --role superadmin
    python scripts/create_user.py --username admin --role superadmin --update
"""
...
from src.user_store import ADMIN_ROLES, UserStore  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--role", default="user", choices=["user", "admin", "superadmin"])
    ap.add_argument(
        "--token-limit",
        type=int,
        default=1_000_000,
    ...
    if args.add_credits:
        ...
        return 0
    # A password is mandatory to create an account; on --update it is optional
    # so that a role change (e.g. promoting an admin to superadmin) does not
    # force a credential reset.
    if not args.password and not (existing and args.update):
        print("error: --password is required when creating a user", file=sys.stderr)
        return 2
    ...
    operator = args.role in ADMIN_ROLES
    if existing:
        updates = dict(
        ...
        record = store.update_user(args.username, **updates)
        if args.provider_key_ref is not None:
            store.billing.update_account(
                args.username,
                provider_key_ref="" if operator else args.provider_key_ref,
            )
        ...
        record = store.create_user(
            ...
            token_limit=args.token_limit,
            features=features,
            plan_type="legacy" if operator else args.plan,
            initial_credits=0 if operator else args.initial_credits,
            markup_percent=args.markup_percent,
            storage_limit_bytes=0 if operator else args.storage_limit_bytes,
            model_policy="" if operator else args.model_policy,
            provider_key_ref=("" if operator else (args.provider_key_ref or "")),
        )
