...
# This is the legacy manual path. Production is deployed by CI
# (.github/workflows/deploy.yml), which takes a verified backup first and can
# roll back; this script does neither, and --with-data overwrites the remote
# data/ and storage/ trees with whatever happens to be on this laptop.
if [ "$REMOTE_APP_DIR" = "/opt/mvp-api" ] && [ "${I_KNOW_THIS_IS_PRODUCTION:-0}" != "1" ]; then
    fail "Refusing to target production ($REMOTE_APP_DIR) from the manual script.
     Use the CI pipeline, or set I_KNOW_THIS_IS_PRODUCTION=1 if you really mean it.
     For the dev environment: REMOTE_APP_DIR=/opt/mvp-api-dev $0 ..."
fi
if [ "$WITH_DATA" -eq 1 ] && [ "${ALLOW_DATA_PUSH:-0}" != "1" ]; then
    fail "--with-data overwrites $REMOTE_APP_DIR/data and /storage with local copies.
     Re-run with ALLOW_DATA_PUSH=1 if that is genuinely what you want."
fi
