...
# Two stacks share this host and each publishes its own Qdrant on a different
# loopback port. Hardcoding 6333 here would make a development backup snapshot
# production's collection.
...
if docker inspect -f '{{.State.Running}}' "$QDRANT_CONTAINER" 2>/dev/null | grep -qx true; then
  ...
  curl -fsS -X POST \
    ...
    "$QDRANT_HTTP/collections/$QDRANT_COLLECTION/snapshots?wait=true" \
    > "$snapshot_response"
