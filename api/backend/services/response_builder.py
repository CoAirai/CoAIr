...
def _clean_answer_text(text: str) -> str:
    """Tidy small LLM slips before the answer reaches the UI — the accidental
    double negative ("does not not contain") reported in testing, and a pipe
    table whose rows all arrived on one line.

    Both the live answer and a conversation reloaded from history come through
    here (backend/api/conversations.py rebuilds the response), so a table that
    was stored broken renders correctly the next time it is opened.
    """
    if not text:
        return text
    ...
    from src.markdown_blocks import normalize_tables
    return normalize_tables(cleaned)
...
def _build_from_single(raw: Dict[str, Any]) -> ChatResponse:
    ...
    sql_artifact = _build_sql_artifact(
        sql, result_data, sources, raw.get("result_columns"),
    )
...
def _build_from_dual(raw: Dict[str, Any]) -> ChatResponse:
    ...
    for provider, prov_ans in answers.items():
        ...
        prov_sql_artifact = _build_sql_artifact(
            prov_sql, prov_result_data, prov_sources, prov_ans.get("result_columns"),
        )
    ...
    answer_text = _normalize_empty_answer(answer_text, related_docs, citations)
    sql_artifact = _build_sql_artifact(
        sql, result_data, sources, first_answer.get("result_columns"),
    )
    cta = _extract_cta(result_data)
...
def _build_sql_artifact(
    ...
    columns = [str(c) for c in result_columns] if isinstance(result_columns, list) else []
    if not columns and preview:
        columns = [str(c) for c in preview[0].keys()]
    return SQLArtifact(
        generated_sql=sql,
