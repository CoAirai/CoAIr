"""FastAPI lifespan: the startup sync every request path assumes has run."""
...
async def lifespan(app: FastAPI):
    ...
    from backend.tasks.event_index_jobs import start_event_index_workers, stop_event_index_workers
    start_ingestion_workers()
    start_report_workers()
    start_forensic_workers()
    start_event_index_workers()
    try:
        yield
    finally:
        ...
        stop_event_index_workers()


def _startup_sync():
    """Synchronous startup for Cloud Run (stateless — GCS is source of truth)."""

    # ── Step 0: Postgres schema + relational stores ───────────
    try:
        from src.database import apply_core_schema, use_postgres
        if use_postgres():
            apply_core_schema()
            ...
            get_ops_store()
            get_commerce_store()
            print("[Startup] Supabase Postgres stores ready")
    except Exception as e:
        print(f"[Startup] Postgres init error: {e}")
        raise
