...
def _enabled(user: UserContext) -> None:
    ...
    if not enabled and not is_admin(user.role):
...
def _parity_enabled(user: UserContext) -> None:
    ...
    if not is_admin(user.role) and not _flag("FORENSIC_PARITY_PUBLIC_V1"):
...
def forensic_status(user: UserContext = Depends(get_current_user)):
    ...
    parity_available = is_admin(user.role) or parity_public
    return {"available": enabled or parity_available,
            ...
            "parity_validation": parity_validation and is_admin(user.role),
