...
@router.get("/event-index/status")
async def get_event_index_status(project: ProjectContext = Depends(get_current_project)):
    """Additive master-memory coverage; document search readiness is independent."""
    from src.master_event_store import get_master_event_store
    return get_master_event_store().project_status(project.project_id)


@router.get("/event-index/metrics")
async def get_event_index_metrics(project: ProjectContext = Depends(get_current_project)):
    from src.master_event_store import get_master_event_store
    return get_master_event_store().project_metrics(project.project_id)
...
@router.post("/files/{file_id}/event-index/retry")
async def retry_file_event_index(
    file_id: str,
    user: UserContext = Depends(get_current_user),
    project: ProjectContext = Depends(require_project_editor),
):
    """Retry only event memory; normal searchable chunks are left untouched."""
    ...
    version = store.active_version_for_document(project.project_id, file_id)
    if not version:
        raise HTTPException(409, "event_canonical_artifact_missing")
    job = store.enqueue_index(
        project.project_id, file_id, str(version["version_id"]), requested_by=user.username,
        force=True,
    )
    get_document_registry().set_event_index(
        file_id, search_status="ready", status=str(job["status"]), partial_reasons=[],
    )
    return {"file_id": file_id, "version_id": version["version_id"], "status": job["status"]}
