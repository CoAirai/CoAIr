...
def _delete_legacy_project_file(file_id: str, project_id: str) -> dict | None:
    ...
    try:
        from src.canonical_artifacts import purge_canonical_artifact
        from src.master_event_store import get_master_event_store
        master = get_master_event_store()
        uris = []
        for doc_id in doc_ids or {file_id}:
            uris.extend(master.purge_document(project_id, doc_id))
            try:
                from src.event_vector_index import get_event_vector_index
                get_event_vector_index().delete_document(
                    project_id=project_id, doc_id=doc_id,
                )
            except Exception:
                # PostgreSQL/canonical deletion is authoritative. A vector
                # outage cannot block source purge; stale vectors cannot
                # resolve through the active-version join and are rebuildable.
                pass
        for uri in uris:
            purge_canonical_artifact(uri)
        result["master_event_versions_deleted"] = len(uris)
    except Exception as exc:
        result["master_event_cleanup_error"] = str(exc)
...
async def list_files(
    ...
    registered = [
        FileInfo(
            ...
            search_status=f.get("search_status", "ready"),
            event_index_status=f.get("event_index_status", "not_started"),
            event_observation_count=f.get("event_observation_count", 0),
            event_cluster_count=f.get("event_cluster_count", 0),
            event_index_version=f.get("event_index_version", ""),
            event_partial_reasons=f.get("event_partial_reasons", []),
