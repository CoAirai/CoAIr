...
from backend.core.orgs import OrgContext, get_current_org
...
def _project_usage(project_id: str, user: UserContext, project: Dict) -> Dict:
    from src.billing_store import get_billing_store
    # Whole-project spend for anyone who administers the project: one of our
    # operators, or the company SuperAdmin reaching it through its organization.
    # An ordinary member still sees only its own consumption.
    company_wide = is_admin(user.role) or project.get("role_source") == "org"
    groups = get_billing_store().usage(
        project_id=project_id, username="" if company_wide else user.username,
    )["groups"]
    out = {
...
def _out(project: Dict, user: UserContext) -> Dict:
    return {**project, "stats": _stats(project["project_id"]),
            "usage": _project_usage(project["project_id"], user, project)}
...
def list_projects(
    ...
    projects = store.list_all() if is_admin(user.role) else store.list_visible(user.username)
...
def create_project(
    ...
    # The organization is taken from the actor, never from the body. Accounts
    # that belong to no company keep the historical behaviour: unlimited,
    # unaffiliated projects.
    if org.org_id and not org.is_platform:
        if org.role != "owner" and not org.policy.get("allow_member_projects"):
            raise HTTPException(403, "org_owner_required")
        limit = int(org.policy.get("project_limit") or 0)
        if limit and store.count_for_org(org.org_id) >= limit:
            raise HTTPException(409, "org_project_limit_reached")
    try:
...
def add_project_member(
    ...
    try:
        ...
        # `cross_org_membership` among others — the store is the choke point, so
        # no route can grant across a company boundary.
        raise HTTPException(422, str(exc)) from exc
    return {"ok": True}
...
def remove_project_member(
    ...
    if project.project_id != project_id:
        raise HTTPException(409, "selected_project_mismatch")
    try:
        removed = store.remove_member(project_id, username)
    except ValueError as exc:
