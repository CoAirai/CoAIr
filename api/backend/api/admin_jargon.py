...
@router.get("/admin/projects/{project_id}/jargon-candidates")
async def list_project_jargon_candidates(project_id: str, status: str = ""):
    from src.master_event_store import get_master_event_store
    try:
        return {"items": get_master_event_store().list_jargon_candidates(
...
@router.post("/admin/projects/{project_id}/jargon-candidates/{candidate_id}")
async def decide_project_jargon_candidate(
    project_id: str, candidate_id: str, payload: CandidateDecision,
):
    from src.master_event_store import get_master_event_store
    item = get_master_event_store().set_jargon_candidate_status(
        project_id=project_id, candidate_id=candidate_id, status=payload.status,
    )
    if not item:
        raise HTTPException(status_code=404, detail="Jargon candidate not found")
    return item
