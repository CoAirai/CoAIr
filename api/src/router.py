...
class QueryRouter:
    ...
    def __init__(self):
        ...
        # Keyed by project: the router is a process-wide singleton, so a single
        # cached block would serve one tenant's topics to the next request.
        self._topic_inventory_cache: Dict[str, Tuple[str, float]] = {}
    ...
    def _get_classification_context(self) -> Tuple[str, str]:
        ...
        Both inventories are project-scoped. They are rendered verbatim into the
        classification prompt — filenames, document summaries, column names and
        sampled cell values — so an unscoped inventory puts one tenant's content
        in another tenant's prompt.
        """
        ...
        # Same allow-list the DATA route uses; None means an unscoped script.
        allowed = self._allowed_data_tables()
        tables = self.data_analyzer.list_tables() if allowed is None else allowed
        table_lines = []
    ...
    def _get_topic_inventory(self, ttl_s: float = 60.0) -> str:
        ...
        now = time.monotonic()
        cached = self._topic_inventory_cache.get(project_id)
        ...
        try:
            lines: List[str] = []
            if not project_id:
                # Unscoped (scripts, legacy callers): the cluster store is global
                # and its labels carry no project, so it is only safe here.
            ...
            # Scoped requests build the block from their own documents'
            # llm_topics (captured at upload). This is also the fallback when
        ...
        self._topic_inventory_cache[project_id] = (block, now)
        return block
    ...
    def _current_question(text: str) -> str:
        ...
        The orchestrator passes the router
    ...
    def _handle_document_query(self, query: str, doc_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        # 3b. Find related Excel/data tables for this topic, scoped the same way
        # the DATA route is: project first, legacy corpus only for the account
        # that predates projects.
        try:
            if doc_ids:
                _allowed = self.data_analyzer.get_tables_for_doc_ids(doc_ids)
            else:
                _allowed = self._allowed_data_tables()
            # None → unrestricted (unscoped script); [] → nothing in scope, so
            # skip enrichment rather than falling through to every table.
    ...
    def _allowed_data_tables(self) -> Optional[List[str]]:
        """The DuckDB tables this request may touch.

        Project is the boundary: the analyzer's table registry is process-wide,
        so scoping decides whose spreadsheets answer the question. The legacy
        corpus tag is consulted only for the one pre-project account whose
        catalog entries carry no project_id — otherwise a fail-closed empty
        list. ``None`` (unrestricted) is reachable only by an unscoped script.
        """
        from .document_rag import _current_user_corpus
        from .project_context import get_current_project_id
        ...
        project_id = get_current_project_id()
        if project_id:
            tables = self.data_analyzer.get_tables_for_project(project_id)
            if tables:
                return tables
        corpus = _current_user_corpus()
        if corpus:
            # Legacy account (e.g. 'edinburgh'): its tables predate projects.
            return self.data_analyzer.get_tables_for_corpus(corpus)
        # Scoped request with no project tables of its own → nothing, not
        # everything. Unscoped scripts (no project, no corpus) keep the old
        # unrestricted behaviour.
        return [] if project_id else None
    ...
    def _handle_data_query(self, query: str, doc_ids: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if doc_ids:
            ...
            allowed_tables = self._allowed_data_tables()
            # Empty list = nothing registered for this scope. Short out cleanly;
            # the SQL path treats an empty allow-list as falsy and would
            # otherwise fall through to every table in the process-wide DuckDB.
            if allowed_tables == []:
                logger.info("   No data tables registered for this scope → clean no-data.")
                return {
                    "query": query,
