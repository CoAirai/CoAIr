    ...
    def search_chunks(self, query: str, top_k: int = 30,
        ...
        if self._ensure_fts():
            try:
                ...
                if doc_ids:
                    where += f" AND doc_id IN ({','.join('?' for _ in doc_ids)})"
        ...
        try:
            ...
            doc_clause = ""
            if doc_ids:
                doc_clause = f" AND doc_id IN ({','.join('?' for _ in doc_ids)})"
            sql = (
                f"SELECT chunk_id, doc_id, file_name, page_number, text, ({score_expr}) AS score "
                f"FROM chunks WHERE {score_expr} > 0{project_clause}{doc_clause} ORDER BY score DESC LIMIT ?"
            )
            tail = [project_id, *(list(doc_ids) if doc_ids else []), top_k]
