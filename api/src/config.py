...
# Z.AI (Zhipu GLM) — OpenAI-compatible chat API. Set LLM_PRIMARY_PROVIDER=zai
# to route chat/chronology/report text through GLM instead of Gemini.
ZAI_API_KEY = os.getenv("ZAI_API_KEY", "")
ZAI_BASE_URL = os.getenv("ZAI_BASE_URL", "https://api.z.ai/api/paas/v4").rstrip("/")
ZAI_MODEL = os.getenv("ZAI_MODEL", "glm-4.7-flash")
ZAI_MODEL_LITE = os.getenv("ZAI_MODEL_LITE", "glm-4.5-flash")
...
# ── Postgres / Supabase (core relational stores) ─────────────
# When set, users/orgs/projects/billing/ops/commerce use Supabase Postgres
# instead of SQLite files under storage/. Vectors stay in Qdrant.
...
def _resolve_app_database_url() -> str:
...
# Ingest-time master chronology. PostgreSQL is selected automatically when
# EVENT_DATABASE_URL is present; local/offline runs use the repository's DuckDB
# adapter. Event extraction is intentionally decoupled from document search.
EVENT_INDEX_ENABLED = os.getenv("EVENT_INDEX_ENABLED", "true").lower() in ("1", "true", "yes")
EVENT_INDEX_MAX_CONCURRENCY = int(os.getenv("EVENT_INDEX_MAX_CONCURRENCY", "1"))
EVENT_HIGH_RISK_VERIFIER_ENABLED = os.getenv(
    "EVENT_HIGH_RISK_VERIFIER_ENABLED", "true"
).lower() in ("1", "true", "yes")
CHRONOLOGY_EVENT_DISCOVERY_MODE = os.getenv(
    "CHRONOLOGY_EVENT_DISCOVERY_MODE", "audit"
).strip().lower()
...
LLM_PRICING = {
    ...
    # Z.AI — GLM-4.7-Flash / GLM-4.5-Flash are free on the public tier (2026).
...
def validate_config() -> tuple[bool, list[str]]:
    ...
    primary = LLM_PRIMARY_PROVIDER if LLM_PRIMARY_PROVIDER in ("gemini", "zai") else "gemini"
    if primary == "zai":
        if not ZAI_API_KEY:
            errors.append(
                "ZAI_API_KEY is not set. Required when LLM_PRIMARY_PROVIDER=zai."
            )
        elif len(ZAI_API_KEY) < 8:
            errors.append("ZAI_API_KEY appears invalid (too short).")
        if not GOOGLE_API_KEY:
            import logging
            logging.warning(
                "GOOGLE_API_KEY is not set. OCR image fallback and multimodal "
                "upload paths still require Gemini."
            )
    ...
    if primary == "gemini" and not ZAI_API_KEY:
        import logging
        logging.warning("ZAI_API_KEY is not set. Set LLM_PRIMARY_PROVIDER=zai to use Z.AI GLM.")
...
def print_config_status():
    ...
    print(f"LLM primary: {LLM_PRIMARY_PROVIDER}")
    print(f"ZAI model: {ZAI_MODEL} / lite: {ZAI_MODEL_LITE}")
