...
from .catalog import _active_corpus, _active_project
...
class DataAnalyzerSQL:
    ...
    SUMMARY_PROMPT = (
        ...
        "13. When the result really is tabular — several rows across the same columns, "
        "a ranking, a breakdown by category — present it as a GitHub-flavoured markdown "
        "table and lead with one or two sentences that read the table for the manager. "
        "Write EVERY row on its own line, header first, then the |---|---| separator row, "
        "then one line per row; a table written on a single line is unreadable. "
        "Never send a bare table with no reading of it (see rule 1)\n\n"
        "GOOD EXAMPLE:\n"
    ...
    def _register_normalized_views(self, table_name: str, df: pd.DataFrame):
        ...
        # Propagate enrichment metadata + scope tags to normalized views
        ...
        for view_name in [raw_name, clean_name]:
            if view_name in self.tables:
                ...
                self.tables[view_name]["project_id"] = base_info.get("project_id", "")
        ...
        try:
            ...
            for table_meta in all_tables:
                ...
                if self.register_parquet_view(parquet_path, table_meta.table_name):
                    ...
                    _project = getattr(table_meta, 'project_id', '') or ''
                    self.tables[table_meta.table_name]["corpus"] = _corpus
                    self.tables[table_meta.table_name]["project_id"] = _project
                    ...
                    for _suffix in ("_raw", "_clean"):
                        ...
                        if _dv in self.tables:
                            ...
                            self.tables[_dv]["project_id"] = _project
        ...
        for source, table_names in by_source.items():
            ...
            try:
                ...
                self.tables[view_name] = {
                    ...
                    "project_id": first_info.get("project_id", ""),
                    **info,
                }
    ...
    def _create_grouped_dataset_views(self):
        ...
        for table_name, info in self.tables.items():
            ...
            # Project and corpus are both part of the grouping key so tables
            # from two tenants never merge into a single UNION view (a grouped
            # view is queryable as one table, so a mixed group is a leak).
            ...
            if target_schema:
                schema_groups.setdefault((project_id, corpus, target_schema), []).append(table_name)
            else:
                no_schema.append(table_name)

        # For tables with target_schema: group by (project, corpus, schema)
        # (ignore filename differences)
        grouped = {}
        for (project_id, corpus, target_schema), table_names in schema_groups.items():
            ...
            group_key = (project_id, corpus, target_schema, target_schema, largest_cols)
            grouped[group_key] = col_sets[largest_cols]

        # For tables without schema: fall back to (project, corpus, dataset_key, columns)
        for table_name in no_schema:
            ...
            corpus = info.get("corpus", "demo")
            project_id = info.get("project_id", "")
            group_key = (project_id, corpus, "", dataset_key, tuple(columns))
            grouped.setdefault(group_key, []).append(table_name)

        for (project_id, corpus, target_schema, dataset_key, _col_sig), table_names in grouped.items():
            if len(table_names) < 2:
                continue
            ...
            # The name must carry the project too, now that grouping is
            # per-project: two projects holding the same dataset would otherwise
            # produce one name, and the second CREATE would silently replace the
            # first project's view.
            scope_suffix = f"_{project_id[:8]}" if project_id else ""
            view_name = sanitize_table_name(
                f"{dataset_key}_{target_schema}_all{scope_suffix}"
            ) + "_grouped"
            unions = " UNION ALL ".join(union_parts)
    ...
    def _generate_sql(self, question: str, table_name: str, provider: str = "gemini") -> str:
        ...
        resp = llm_client.generate_text(prompt, system=system, task_type="sql",
    ...
    def get_tables_for_project(self, project_id: Optional[str]) -> List[str]:
        """DuckDB table names belonging to ``project_id``.

        Fail-closed: an empty scope returns ``[]``, never "everything". The
        DuckDB registry is process-wide and holds every project's parquet, so a
        permissive default here means one tenant's question is answered from
        another tenant's spreadsheet. Compare ``get_tables_for_corpus`` below,
        which still returns ``None`` for the one legacy account that predates
        projects.
        """
