...
def _counter_evidence_paragraph(
    doc: Document,
    claims: Iterable[VerifiedClaim],
    sources: Dict[str, EvidenceItem],
    exhibits: Dict[str, int],
    ...
    counter_ids = list(dict.fromkeys(
        source_id for claim in claims for source_id in claim.counter_source_ids
    ))
    if not counter_ids:
        return None
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
    p.paragraph_format.left_indent = Cm(1.905)
    p.add_run("Counter-evidence:").bold = True
    p.add_run(" Countervailing records were identified")
    for source_id in counter_ids:
        ...
        if source is None:
            ...
            continue
        exhibit = exhibits.setdefault(source.source_id, len(exhibits) + 1)
        footnotes.add(p, source.footnote_text(exhibit, "cited as counter-evidence"))
    p.add_run(".")
    return p
...
def build_ai_chronology_docx(
    *,
    ...
    for position, entry in enumerate(ordered, 1):
        ...
        _counter_evidence_paragraph(
            doc, claims, sources, exhibits, footnotes, unresolved,
        )
