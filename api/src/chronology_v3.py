...
class ChronologyModel(BaseModel):
    ...
    entries: List[FinalEventModel] = Field(default_factory=list)
...
class EventSynthesisModel(BaseModel):
    entries: List[FinalEventModel] = Field(default_factory=list, max_length=SYNTHESIS_BATCH_EVENTS)
        ...
        try:
            ...
            if save_step:
                save_step(
                    ...
                    classify_report_error(exc),
                )
            if not _is_structured_output_error(exc):
                raise
            raise RuntimeError(classify_report_error(exc)) from exc
    ...
    *, map_skeleton: Sequence[str] = (),
    ...
    def run(batch: List[EvidenceItem], key: str, depth: int = 0) -> None:
        ...
        try:
            ...
            if save_step:
                save_step(
                    key, input_hash, "failed", None, classify_report_error(exc),
                )
            if not _is_structured_output_error(exc):
                raise
            raise RuntimeError(classify_report_error(exc)) from exc
...
def aggregate_events(
    ...
    # Exact duplicate removal is deterministic; every original observation and
    # its own provenance remain available to synthesis.
    unique: Dict[Tuple[str, str, str], Dict] = {}
    for raw in events:
        event = deepcopy(raw)
        key = (
            str(event.get("event_date") or ""), str(event.get("actor") or "").casefold(),
            re.sub(r"\W+", " ", str(event.get("action") or "").casefold()).strip(),
        )
        existing_observations = deepcopy(event.pop("observations", []))
        observation = deepcopy(event)
...
def _event_source_ids(event: Dict) -> set[str]:
    return {
        str(source_id) for observation in event.get("observations", [event])
        for source_id in (
            list(observation.get("supporting_source_ids", []))
            + list(observation.get("counter_source_ids", []))
        )
    }
    ...
    for group in claim_groups:
        for claim in group.get("claims", []):
            ...
            if not ids or any(source_id not in by_id for source_id in ids):
                return False
            counter_ids = [
                str(source_id) for source_id in claim.get("counter_source_ids", [])
            ]
            if any(source_id not in by_id for source_id in counter_ids):
                return False
            source_text = " ".join(by_id[source_id] for source_id in ids)
...
def synthesize(
    ...
    used = {
        str(source_id) for event in events
        for observation in event.get("observations", [event])
        for source_id in (
            list(observation.get("supporting_source_ids", []))
            + list(observation.get("counter_source_ids", []))
        )
    ...
    def call_step(
        ...
        validation_model: type[BaseModel], semantic_validator,
    ) -> Dict:
        ...
        if previous and previous.get("status") == "ready":
            ...
            try:
                ...
                candidate = {}
            if candidate and semantic_validator(candidate):
                return candidate
        if save_step:
            save_step(step_key, input_hash, "processing", None, "")
        try:
            response = generate_response_json(
                prompt, system=prompts["system"], schema=validation_model.model_json_schema(),
                schema_name=schema_name, validation_model=validation_model,
                task_type="chronology_synthesis", thinking_level="medium",
                max_tokens=32_768, prompt_version=prompts["version"],
                cache_key=schema_name, cache_context=f"{cache_context}:{input_hash}",
                ttl_s=0, semantic_validator=semantic_validator,
            )
        except Exception as exc:
            if save_step:
        ...
        try:
            ...
            if not semantic_validator(output):
    ...
    overview_hash = _hash({
        "events": ordered, "evidence": [asdict(item) for item in cited],
        "prompt": chronology_v3_prompt_hash(), "schema": OverviewModel.model_json_schema(),
    })
    overview = call_step(
        "synthesis:overview", overview_hash,
        f"{base}\n\nCreate only the sourced overview; do not return dated events.\n\n"
        f"VERIFIED LEDGER:\n{json.dumps(ordered, ensure_ascii=False)}\n\n"
        f"EVIDENCE BEGIN\n{evidence_markdown(cited)}\nEVIDENCE END",
        schema_name="chronology_v3_overview", validation_model=OverviewModel,
        semantic_validator=lambda value: _final_valid(
            {"overview_claims": value.get("overview_claims", []), "entries": []},
    ...
    for batch_number, batch in enumerate(_synthesis_batches(ordered, evidence), 1):
        ...
        batch_source_ids = {
            str(source_id) for event in batch
            for observation in event.get("observations", [event])
            for source_id in (
                list(observation.get("supporting_source_ids", []))
                + list(observation.get("counter_source_ids", []))
        ...
        batch_evidence = [item for item in evidence if item.source_id in batch_source_ids]
        ...
        def batch_valid(value: Dict, expected: set[str] = expected_ids) -> bool:
            ...
            return (
                ...
                and _final_valid(
                    {"overview_claims": [], "entries": values},
                    batch_evidence, enforce_style=False,
                )
            )

        input_hash = _hash({
            "events": batch, "evidence": [asdict(item) for item in batch_evidence],
            "prompt": chronology_v3_prompt_hash(),
            "schema": EventSynthesisModel.model_json_schema(),
        })
        output = call_step(
            f"synthesis:event:{batch_number}", input_hash,
            ...
            f"VERIFIED LEDGER BATCH:\n{json.dumps(batch, ensure_ascii=False)}\n\n"
            f"EVIDENCE BEGIN\n{evidence_markdown(batch_evidence)}\nEVIDENCE END",
            schema_name="chronology_v3_event_batch", validation_model=EventSynthesisModel,
            semantic_validator=batch_valid,
        )
    ...
    if not _style_valid(result):
        raise ValueError("model_output_invalid")
    return ChronologyModel.model_validate(result).model_dump()
...
def verify_and_repair(
    ...
    for offset in range(0, len(claims), VERIFICATION_BATCH_CLAIMS):
        ...
        used = {
            str(source_id) for claim in batch for source_id in (
                list(claim.get("source_ids", []))
                + list(claim.get("counter_source_ids", []))
            )
        }
        cited = [item for item in evidence if item.source_id in used]
        input_hash = _hash({
            ...
            "prompt": chronology_v3_prompt_hash(),
            "schema": VerificationModel.model_json_schema(),
        })
        ...
        if previous and previous.get("status") == "ready":
            ...
            def valid_decisions(value: Dict, expected: set[str] = refs) -> bool:
            ...
            try:
                raw = generate_response_json(
                    ...
                    semantic_validator=valid_decisions,
                ).raw
            except Exception as exc:
                if save_step:
                    save_step(step_key, input_hash, "failed", None, classify_report_error(exc))
                raise
            if save_step:
                save_step(step_key, input_hash, "ready", dict(raw), "")
        try:
            ...
            if save_step:
                save_step(step_key, input_hash, "failed", None, classify_report_error(exc))
            raise ValueError("model_output_invalid") from exc
        decisions_list.extend(raw.get("decisions", []))

    all_refs = {str(claim["claim_ref"]) for claim in claims}
    if len(decisions_list) != len(all_refs) or {
        str(item.get("claim_ref") or "") for item in decisions_list
    } != all_refs:
        raise ValueError("source_verification_failed")

    flagged = [item for item in decisions_list
               if item.get("decision") in ("QUALIFY", "SPLIT")]
    repairs_by_ref: Dict[str, List[str]] = {}
    for offset in range(0, len(flagged), REPAIR_BATCH_CLAIMS):
        decision_batch = flagged[offset:offset + REPAIR_BATCH_CLAIMS]
        flagged_refs = {str(item["claim_ref"]) for item in decision_batch}
        ...
        used = {
            str(source_id) for claim in flagged_claims for source_id in (
                list(claim.get("source_ids", []))
                + list(claim.get("counter_source_ids", []))
            )
        }
        ...
        def repairs_valid(value: Dict, expected: set[str] = flagged_refs) -> bool:
            repairs = value.get("repairs", [])
            if {str(item.get("claim_ref") or "") for item in repairs} != expected:
                return False
            ...
            for item in repairs:
                ...
                for text in item.get("texts", []):
                    probe["overview_claims"].append({
                        ...
                        "counter_source_ids": original.get("counter_source_ids", []),
                    })
            return _final_valid(
        ...
        input_hash = _hash({
            "claims": flagged_claims, "decisions": decision_batch,
            "evidence": [asdict(item) for item in cited],
            "prompt": chronology_v3_prompt_hash(), "schema": RepairModel.model_json_schema(),
        })
        step_key = f"repair:batch:{offset // REPAIR_BATCH_CLAIMS + 1}"
        previous = load_step(step_key, input_hash) if load_step else None
        if previous and previous.get("status") == "ready":
            repaired = dict(previous.get("output") or {})
            try:
                RepairModel.model_validate(repaired)
            except Exception:
                repaired = {}
            if repaired and not repairs_valid(repaired):
                repaired = {}
        else:
            repaired = {}
        if not repaired:
            ...
            try:
                repaired = generate_response_json(
                    ...
                    f"{json.dumps(decision_batch, ensure_ascii=False)}\n\nEVIDENCE BEGIN\n"
                    f"{evidence_markdown(cited)}\nEVIDENCE END",
                    ...
                    cache_context=f"{cache_context}:repair:{input_hash}", ttl_s=0,
            ...
            try:
                ...
                raise ValueError("model_output_invalid") from exc
            if save_step:
                save_step(step_key, input_hash, "ready", dict(repaired), "")
        repairs_by_ref.update({
            str(item["claim_ref"]): list(item.get("texts", []))
            for item in repaired.get("repairs", [])
        })

    decisions = {str(item["claim_ref"]): item for item in decisions_list}
    ...
    if (not result["overview_claims"] or not result["entries"]
            or not _final_valid(result, evidence, enforce_style=False)):
        raise ValueError("source_verification_failed")
    return result, {"decisions": decisions_list, "repairs": repairs_by_ref}


def _issued_source_ids(entries: Sequence[ChronologyEntry]) -> set[str]:
    return {
        source_id for entry in entries for claim in entry.claims
        for source_id in [*claim.source_ids, *claim.counter_source_ids]
    }
...
def generate_chronology_v3(
    ...
    **_ignored,
    ...
    if CHRONOLOGY_EVENT_DISCOVERY_MODE == "primary_fallback":
        from .ai_reports import retrieve_evidence
        from .chronology_discovery import discover_chronology_evidence
        discovery = discover_chronology_evidence(
            project_id=project_id, prepared=prepared, date_from=date_from, date_to=date_to,
            fallback=lambda queries, doc_ids: retrieve_evidence(
                project_id, queries, doc_ids=doc_ids,
            ),
        )
        selection = select_pack(discovery.evidence, facets=COVERAGE_FACETS)
        evidence = selection.evidence
        index_records = {row.doc_id: row for row in get_document_index().list_project(project_id)}
        score_by_doc: Dict[str, float] = {}
        for item in evidence:
            score_by_doc[item.doc_id] = max(score_by_doc.get(item.doc_id, 0), float(item.score or 0))
        selected = []
        for doc_id, score in sorted(score_by_doc.items(), key=lambda item: -item[1]):
            record = index_records.get(doc_id)
            if record:
                selected.append(CandidateDocument(
                    doc_id=record.doc_id, file_name=record.file_name, reference=record.reference,
                    title=record.title, description=record.description,
                    document_family=record.document_family, metadata_date=record.metadata_date,
                    metadata_date_source=record.metadata_date_source, ocr_quality=record.ocr_quality,
                    score=score, role="primary", reasons=["master_event_observation"],
                    content_hash=record.content_hash,
                ))
            else:
                item = next(value for value in evidence if value.doc_id == doc_id)
                selected.append(CandidateDocument(
                    doc_id=doc_id, file_name=item.file_name, reference="", title=item.title,
                    description="", document_family="other", metadata_date="",
                    metadata_date_source="unknown", ocr_quality="good", score=score,
                    role="primary", reasons=["master_event_observation"], content_hash="",
                ))
        research_audit = {
            "event_discovery": discovery.audit,
            "warnings": list(discovery.audit.get("partial_reasons", [])),
            "coverage": coverage_matrix(evidence), "selection": selection.stats,
            "map_skeleton": [], "research_leads": [],
            "selected": [asdict(item) for item in selected],
        }
    else:
        ...
        if CHRONOLOGY_EVENT_DISCOVERY_MODE == "audit":
            try:
                from .chronology_discovery import discover_chronology_evidence
                audit_discovery = discover_chronology_evidence(
                    project_id=project_id, prepared=prepared,
                    date_from=date_from, date_to=date_to, fallback=None,
                )
                research_audit["event_discovery"] = audit_discovery.audit
            except Exception:
                pass
    if not evidence:
        raise ValueError("no_evidence")
    ...
    aggregation_hash = _hash({
        "events": events, "evidence": evidence_hash,
        "prompt": chronology_v3_prompt_hash(),
        "schema": ExtractionModel.model_json_schema(),
        "contract": PIPELINE_CONTRACT_REVISION,
    })
    aggregation_step = load_step("aggregation", aggregation_hash) if load_step else None
    cached_events = list((aggregation_step.get("output") or {}).get("entries", [])) if (
        aggregation_step and aggregation_step.get("status") == "ready"
    ) else []
    if cached_events and all(event.get("cluster_id") for event in cached_events):
    ...
    synthesized_event_count = len(chronology.get("entries", []))
    ...
    for index, event in enumerate(chronology["entries"], 2):
        entries.append(ChronologyEntry(
            ...
            date_source=event.get("date_source", "unresolved"),
    ...
    issued_source_ids = _issued_source_ids(entries)
    issued_evidence = [
    ...
    return {
        ...
        "audit": {
            **asdict(word_audit), "candidate_clusters": len(events),
            "synthesized_events": synthesized_event_count,
            "verifier_removed_events": synthesized_event_count - (len(entries) - 1),
            "issued_events": len(entries) - 1,
        }, "docx": blob,
        ...
        "partial_reasons": list(research_audit["warnings"]),
