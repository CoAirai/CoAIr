...
from .config import (
    ...
    ZAI_API_KEY, ZAI_BASE_URL, ZAI_MODEL, ZAI_MODEL_LITE,
...
def primary_llm_provider() -> str:
    """Return the active text provider: gemini (default) or zai when configured."""
    preferred = LLM_PRIMARY_PROVIDER if LLM_PRIMARY_PROVIDER in ("gemini", "zai") else "gemini"
    if preferred == "zai" and ZAI_API_KEY:
        return "zai"
    if preferred == "zai" and not ZAI_API_KEY:
        logger.warning(
            "[LLMClient] LLM_PRIMARY_PROVIDER=zai but ZAI_API_KEY is missing — "
            "falling back to gemini"
        )
    return "gemini"


def _model_for_task(task_type: str, provider: Optional[str] = None) -> str:
    """Keep user research on the quality model; reserve the lite model for ingestion."""
    active = provider or primary_llm_provider()
    if active == "zai":
        return ZAI_MODEL_LITE if _is_ingestion_task(task_type) else ZAI_MODEL
...
def effective_providers(providers: List[str]) -> List[str]:
    """Return the single active provider; never fan out duplicate calls."""
    return [primary_llm_provider()]
...
def generate_text(
    ...
    *,
    ...
    model = _model_for_task(task_type, provider)
    ...
    if json_mode and provider in ("zai", "openai", "claude") and not response_schema:
    ...
    try:
        ...
        if _tr is not None:
            ...
            if (not chronology_budget_active_var.get()
                    and _real_calls >= MAX_LLM_CALLS_PER_QUERY
                    and provider in ("gemini", "zai")
    ...
    for attempt in range(1 + retry_count):
        try:
            ...
            if use_native_gemini or use_gemini_thinking:
                ...
                text = text.strip()
            # Use chat() for OpenAI-compatible providers (proper system prompt handling)
            elif provider in ("openai", "claude", "zai") and system:
            ...
            # ── Thinking burned the whole output budget ──
            # Gemini 3 draws reasoning tokens from max_output_tokens, so a tight
            # budget can return zero visible text with finish_reason=MAX_TOKENS.
            # Retry once with thinking off and a wider budget before failing.
            if (not text and finish_reason == "MAX_TOKENS"
                    and not thinking_degraded and attempt < retry_count):
                thinking_degraded = True
                widened = clamp_output_tokens(model, max_tokens * 4, profile)
                logger.warning(
                    f"[LLMClient] {provider}/{model} empty output at MAX_TOKENS "
                    f"({max_tokens} tok, thinking={thinking_level}) — retrying with "
                    f"minimal thinking and {widened} tok"
                )
                max_tokens = widened
                thinking_level = "minimal"
                thinking = 0
                ...
                if not use_native_gemini:
                    llm, model = create_llm(
                        provider, temperature, max_tokens, thinking=0, model=model,
                    )
                continue
...
def generate_response_json(
    ...
    *,
    ...
    """Generate quality-critical structured reports.

    Gemini uses native JSON schema when available. Z.AI and other OpenAI-compatible
    providers receive the schema in the system prompt and return parsed JSON text.
    """
    active = primary_llm_provider()
    if active == "zai":
        schema_hint = json.dumps(schema, ensure_ascii=False, sort_keys=True)
        augmented_system = (
        ...
        response = generate_text(
            ...
            model=_model_for_task(task_type, "zai"),
            provider="zai",
            json_mode=True,
