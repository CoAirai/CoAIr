...
const SecurityPage = () => {
    ...
    const onAddIp = (event: FormEvent<HTMLFormElement>) => {
    ...
    const onCreateKey = (event: FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        const result = createApiKey(keyLabel);
    ...
    return (
        <div className="space-y-6">
            ...
            <section className="rounded-2xl border border-stroke-soft-200 bg-white-0 p-5">
                ...
                <p className="mt-1 text-label-xs text-sub-600">
                    Automatically sign out inactive admin sessions after the
                    selected duration.
                </p>
                <label className="mt-4 block max-w-xs">
                    ...
                    <select
                        ...
                        onChange={(event) =>
                            updateSessionTimeout(Number(event.target.value))
                        }
                ...
                <div className="space-y-2 p-5">
                    {security.ipAllowlist.length === 0 ? (
                        <p className="text-label-sm text-sub-600">
                            No IPs configured — all addresses are allowed.
                        </p>
                    ) : (
            ...
            <section className="rounded-2xl border border-stroke-soft-200 bg-white-0">
                <div className="border-b border-stroke-soft-200 p-5">
                    ...
                    <p className="mt-1 text-label-xs text-sub-600">
                        Create keys for platform integrations. Each key is shown
                        in full only once at creation.
                    </p>
                </div>
                                ...
                                <tr key={key.id} className="text-label-sm">
                                    ...
                                    <td className="px-5 py-4">
                                        <button
                                            ...
                                            className="h-8 rounded-lg px-3 text-label-xs text-red-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40"
                                        >
                                            Revoke
                                        </button>
                                    </td>
